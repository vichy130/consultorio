...
comida varchar(200), cena varchar(200), entreComidas varchar(200), vasoAguaDia int(1), otrosLiquidos varchar(200), intolerancias varchar(1000), orinaDia varchar(200),
 ...
 enfermedadesPadecidas varchar(2000), enfermedadesPresentes varchar(2000), firmaPaciente blob, firmaUsuario blob, hora time, usuario varchar(20));

create table antecedentesFamilia (id int(10) primary key auto_increment, familiar varchar(45), comentarios varchar(2000), ficha int(10));
...
alter table antecedentesPaciente add constraint fk_antecedentesPaciente foreign key (ficha) references ficha(id) on update cascade on delete cascade;
...
insert into ficha (paciente,tipoSangre,quienRecomendo,embarazo,partos,cesareas,abortos,muertos,enfs,fuma,cigarrosDia,fumaAntiguedad,alcohol,alcFrecuencia,alcoholCantidad,alcoholTipos,adicciones,alergias,desayuno,comida,
